      ...
      <better-scroll :options="{ scrollX: true, scrollY: false }" ref="bsScroll">
        <div class="tags-scroll-inner">
          ...
          >
            ...
            <el-icon
              v-if="!isAffix(tag)"
              @click.prevent.stop="(e:any) => closeSelectedTag(e, tag)"
              class="tag-icon"
            >
...
const closeSelectedTag = async(event:any, view:any) => {
  if (tags.value.get(view.path)) {
    tags.value.delete(view.path)
  }
  const { visitedViews } = (await TagsViewStore.delView(view)) as any
  if (isActive(view)) {
...
function handleScrollAction(currentTag:any) {
  const scrollContainerRect = scrollContainer.value.getBoundingClientRect()
  const { left: currx, width: currentWidth } = currentTag.getBoundingClientRect()
...
function moveToCurrentTag() {
  nextTick(() => {
    for (const [, tag] of tags.value) {
      const path = tag.attributes.path.value
      if (path === route.path) {
        const fullPath = tag.attributes.fullPath.value
        // 移动
...
onMounted(() => {
  ...
  router.beforeEach(async(to, from, next) => {
    if (
      ...
      const whiteList = ['/error/404', '/error/401']
